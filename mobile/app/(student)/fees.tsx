...
type Wallet = { balance: number; lastUpdated?: string }

export default function Fees() {
  const feesQ   = useApiQuery<FeeRecord[]>(['fees'], '/finance/fees')
  const walletQ = useApiQuery<Wallet>(['wallet'], '/finance/wallet')

  if (feesQ.isLoading) return <Spinner />
  ...
  return (
    <View className="flex-1 bg-ink-50 dark:bg-ink-950">
      <ScreenHeader title="Fees" back variant="large" />
      <ScrollView contentContainerStyle={{ padding: 20, paddingBottom: 80 }}>
        {/* Hero balance card */}
        <View
          style={{ backgroundColor: palette.brand[600], borderRadius: 24, padding: 20 }}
        >
          ...
          <View className="flex-row gap-3 mt-5">
            <View className="flex-1 bg-white/15 rounded-2xl p-3">
              <Text className="text-white/70 text-[11px]">Wallet</Text>
              <Text className="text-white text-lg font-bold">
                ₹{(walletQ.data?.balance ?? 0).toLocaleString('en-IN')}
              </Text>
            </View>
            <View className="flex-1 bg-white/15 rounded-2xl p-3">
              <Text className="text-white/70 text-[11px]">Pending bills</Text>
              <Text className="text-white text-lg font-bold">
        ...
        {fees.length === 0 ? (
          <EmptyState icon="payments" title="No bills" message="Your fee bills will appear here." />
        ) : (
                ...
                <View className="flex-row items-center justify-between">
                  <View className="flex-1">
                    ...
                    <Text className="text-xs text-ink-500 dark:text-ink-400 mt-1">
                      Due {format(parseISO(f.dueDate), 'd MMM yyyy')}
                    </Text>
                  </View>
                  <View className="items-end">
                    ...
                    <Badge
                      label={f.status}
                      tone={
                        f.status === 'PAID' ? 'success' : f.status === 'OVERDUE' ? 'danger' : 'warning'
                      }
                    />
                  </View>
                </View>
