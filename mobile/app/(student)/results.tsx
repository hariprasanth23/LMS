...
const GRADE_TONES: Record<string, string> = {
  O: '#16A34A', 'A+': '#22C55E', A: '#65A30D', 'B+': '#0EA5E9',
  B: '#0284C7', C: '#F59E0B', D: '#F97316', F: '#EF4444',
}

export default function Results() {
  const { data, isLoading } = useApiQuery<Grade[]>(['results'], '/examination/grades/me')
  ...
  return (
    <View className="flex-1 bg-ink-50 dark:bg-ink-950">
      <ScreenHeader title="Results" variant="large" />
      <ScrollView contentContainerStyle={{ padding: 20, paddingBottom: 80 }}>
        {latest ? (
          ...
          <EmptyState icon="grade" title="No results yet" message="Your grade sheets will appear after exams." />
        )}

        {grades.map((g) => (
          <View key={g.semester} className="mt-6">
            <View className="flex-row items-center justify-between mb-3">
              <Text className="text-base font-semi text-ink-900 dark:text-ink-50">
                Semester {g.semester}
              </Text>
              ...
              {g.courses.map((c) => (
                <Card key={c.code}>
                  <View className="flex-row items-center">
                    ...
                    >
                      <Text
                        style={{ color: GRADE_TONES[c.grade] ?? palette.brand[500] }}
