  ...
  return (
    <View className="flex-1 bg-ink-50 dark:bg-ink-950">
      ...
      <ScrollView contentContainerStyle={{ padding: 20, paddingBottom: 80 }}>
        <View style={{ backgroundColor: PORTAL_COLORS.PARENT, borderRadius: 24 }} className="p-5 flex-row items-center">
          <Avatar name={user?.name} size={64} tone="#FFFFFF" />
          <View className="ml-4 flex-1">
            <Text className="text-white text-xl font-bold" numberOfLines={1}>{user?.name ?? 'Parent'}</Text>
        ...
        <Text className="mt-6 mb-2 text-xs font-semi text-ink-500 uppercase tracking-wider">Account</Text>
        <Card>
          {[
            { icon: 'phone' as const,           label: 'Phone', value: user?.phone ?? '—' },
            { icon: 'alternate-email' as const, label: 'Email', value: user?.email ?? '—' },
            { icon: 'family-restroom' as const, label: 'Ward',  value: 'Linked via roll number' },
          ].map((r, i, arr) => (
            <View key={r.label}>
