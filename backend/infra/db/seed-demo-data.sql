-- ─────────────────────────────────────────────────────────────────────────────
-- Demo seed for the domain services (departments, students, employees,
-- courses, leave balances).
--
-- Assumes:
--   1. Flyway migrations already ran on each service DB (tables exist).
--   2. auth/DemoDataSeeder already ran (users exist with the fixed UUIDs
--      referenced below).
--
-- Run from the host with:
--   PGPASSWORD=changeme psql -h localhost -p 5432 -U lmsadmin -f seed-demo-data.sql
--
-- Or via docker exec:
--   docker compose exec -T postgres psql -U lmsadmin -f /tmp/seed-demo-data.sql
--
...
INSERT INTO departments (name, code, description, email, phone, location, established_year, total_seats) VALUES
  ...
  ('Mechanical Engineering',         'MECH',  'Thermodynamics, manufacturing',         'mech@sample.edu',  '+91-9000000003', 'Block C', 1980, 180),
...
SELECT u::uuid, lt, td, ud, EXTRACT(year FROM CURRENT_DATE)::int
FROM (VALUES
