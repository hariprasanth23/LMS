 ...
 *
 * <p>Gateway adds {@code X-Internal-Ts} (epoch ms) and {@code X-Internal-Sig}
 * (HMAC-SHA256 over {@code ts:path:userId}). We recompute and compare in
 * constant time. Replay window is 60 seconds by default.
 *
 * <p>Skips for actuator and health endpoints — those are reached directly
 * by the orchestrator's liveness probes, not via the gateway.
 *
 * <p>{@code app.internal.enforce=false} (default in `local` profile) makes
 * this filter pass-through for local dev where the gateway isn't running.
 */
@Slf4j
@Component
@Order(Ordered.HIGHEST_PRECEDENCE + 10)   // before Spring Security
public class InternalAuthFilter implements Filter {

    private static final Set<String> SKIP_PREFIXES = Set.of(
            "/actuator", "/api/_health"
    );

    @Value("${app.internal.secret}")
    private String secret;

    @Value("${app.internal.enforce:true}")
    private boolean enforce;

    @Value("${app.internal.max-age-seconds:60}")
    ...
    void init() {
        if (!enforce) {
            log.warn("InternalAuthFilter: ENFORCE=false — gateway signature is NOT being checked");
            return;
        }
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
        ...
        if (ts == null || sig == null) {
            reject(resp, "Missing internal signature headers");
            return;
        }
        ...
        if (ageSec > maxAgeSeconds) {
            reject(resp, "Internal signature expired (age=" + ageSec + "s)");
            return;
        }
        ...
        if (!constantTimeEquals(sig, expectedHex)) {
            reject(resp, "Bad internal signature");
            return;
        }
        chain.doFilter(request, response);
    }
